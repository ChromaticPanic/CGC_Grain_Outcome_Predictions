...
class CSVBorderReader:
    ...
    def __processInput(self, inputRow):
        ...
        currProv = (
            -1
        )  # set to -1 because we immedatiely will increment on our first iteration
        combinedCols = (
            ""  # used to combine columns in instance of incomplete border data
        )

        # input is a 2D array: primary index refers to rows, secondary index refers to columns
        # starts at 1 to skip over headers at row 0
        for i in range(1, len(inputRow)):
            if self.__isProvince(
                inputRow[i][0]
            ):  # found a province, store name and borders (b/c of CSV limitations may not be complete)
                ...
                if inputRow[i][0]:  # if data is not empty proceed with processing
                    ...
                    for col in inputRow[
                        i
                    ]:  # add all columns together (start of data is all housed in a single cell)
    ...
    def __conformBorders(self, borders):
        for i in range(0, len(borders)):
            borders[i] = borders[i].replace(
                '"', ""
            )  # some rows contained ", remove them
            borders[i] = self.__enforceEndParenthesis(
                borders[i]
            )  # ensures all border data ends with the correct number of parenthesis (3)
    ...
    def __enforceEndParenthesis(self, border):
        ...
        numToCorrect = (
            numParenthesis - 3
        )  # all data will contain atleast 3 because of data processing (line 51)
        if (
            numToCorrect
        ):  # If we have parenthesis to correct (string[:0] has undesired behavior)
