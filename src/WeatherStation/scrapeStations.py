...
import os, sys, numpy, pandas, typing, geopandas, sqlalchemy 
...
def storeLastUpdated(stationID: str, lastUpdated: numpy.datetime64, queryHandler: QueryHandler, db: DataService, updatdUntil: numpy.datetime64):
    if numpy.isnat(numpy.datetime64(lastUpdated)):  # lastUpdated is None if its respective station has not had its data pulled yet
...
def getStations(prov: str, db: DataService, queryHandler: QueryHandler, conn: sqlalchemy.engine.Connection) -> typing.Tuple[pandas.DataFrame, list]:
