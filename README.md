...
Our current environment is composed of docker containers housing PGADMIN4, PostgreSQL and Jupyter notebooks. To create/restart this environment use setup.bat from the project directory.
...
- PGADMIN:
    ...
    3. use the following command ```docker inspect {containnerID}``` note that the containerID can be located within Docker Desktop (which has built in copy functionality)
    3. connect to postgres instance using the IPAddress found from step 3
